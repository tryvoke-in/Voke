    ...
    <div className="h-screen w-screen bg-[#080B11] text-slate-100 flex flex-col overflow-hidden relative font-sans select-none">
      ...
      >
        ...
        >
          ...
          {/* LEFT STAGES CONTAINER (STEP 1, STEP 2, STEP 3) - ALWAYS CENTERED & ROCK-SOLID (NEVER SHIFTS) */}
              ...
              {INTERVIEW_TYPES.map((typeItem) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`p-4 rounded-3xl border flex items-center gap-3.5 transition-all duration-200 min-w-[230px] shadow-2xl backdrop-blur-2xl ${
                      isActive ? 'cursor-pointer' : 'opacity-40 cursor-not-allowed border-slate-800 bg-slate-950/40'
                    } ${
                      isSelected
                        ? 'border-amber-500/80 bg-gradient-to-r from-amber-500/20 to-slate-900 text-white ring-2 ring-amber-500/40 shadow-xl shadow-amber-500/10 scale-[1.02]'
                        : isActive
                    ...
                    {TOP_COMPANIES.slice(0, 5).map((company) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className={`px-4 py-3 rounded-2xl border flex items-center gap-3 cursor-pointer transition-all duration-200 min-w-[220px] shadow-xl backdrop-blur-2xl ${
                            isSelected
                              ? 'border-indigo-500/80 bg-gradient-to-r from-indigo-600/20 to-slate-900 text-white ring-2 ring-indigo-500/40 shadow-indigo-500/20 scale-[1.02]'
                    ...
                    {ELITE_ROLES.map((role) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className={`px-4 py-3.5 rounded-2xl border flex items-center gap-3 cursor-pointer transition-all duration-200 min-w-[230px] shadow-xl backdrop-blur-2xl ${
                            isSelected
                              ? 'border-emerald-500/80 bg-gradient-to-r from-emerald-600/20 to-slate-900 text-white ring-2 ring-emerald-500/40 shadow-emerald-500/20 scale-[1.02]'
            ...
            {selectedType && selectedCompany && selectedRole && (
              <>
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0, scaleX: 0 }}
                  transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
                  className="w-14 md:w-16 h-[520px] relative shrink-0 origin-left self-start mt-20"
                >
                  <svg className="w-full h-full overflow-visible">
                    <path d="M 0 170 C 30 170, 20 40, 56 40" fill="none" stroke="rgba(16, 185, 129, 0.85)" strokeWidth="3.5" />
                    <path d="M 0 170 C 30 170, 20 170, 56 170" fill="none" stroke="rgba(16, 185, 129, 0.7)" strokeWidth="3" />
                    <path d="M 0 170 C 30 170, 20 300, 56 300" fill="none" stroke="rgba(16, 185, 129, 0.7)" strokeWidth="3" />
                    <path d="M 0 170 C 30 170, 20 430, 56 430" fill="none" stroke="rgba(16, 185, 129, 0.7)" strokeWidth="3" />
                  </svg>
                </motion.div>
                ...
                <motion.div
                  ...
                  className="flex flex-col gap-4 shrink-0 z-10 w-[640px] md:w-[720px] self-start pt-2 pb-16"
                >
                  ...
                  {rounds.map((roundDef) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-5 md:p-5.5 rounded-3xl border transition-all duration-300 shadow-2xl space-y-3.5 backdrop-blur-2xl hover:scale-[1.01] ${
                          isPassed
                            ? 'border-emerald-500/40 bg-gradient-to-b from-emerald-950/20 via-slate-900/90 to-slate-950/90 shadow-emerald-950/20 ring-1 ring-emerald-500/20'
                            : isFailed
                            ? 'border-rose-500/40 bg-gradient-to-b from-rose-950/20 via-slate-900/90 to-slate-950/90 shadow-rose-950/20 ring-1 ring-rose-500/20'
                            : isUnlocked
                            ? 'border-amber-500/50 bg-gradient-to-b from-amber-950/20 via-slate-900/90 to-slate-950/90 shadow-amber-950/20 ring-2 ring-amber-500/20'
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className={`w-2 h-2 rounded-full inline-block ${
                              isPassed ? 'bg-emerald-400' : isFailed ? 'bg-rose-400' : isUnlocked ? 'bg-amber-400 animate-pulse' : 'bg-slate-600'
                            }`} />
                          ...
                          {isPassed && (
                            <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-mono font-black bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 shadow-sm shadow-emerald-500/10 flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3 text-emerald-400" /> PASSED
                            </span>
                          )}
                          {isFailed && (
                            <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-mono font-black bg-rose-500/15 text-rose-300 border border-rose-500/30 shadow-sm shadow-rose-500/10 flex items-center gap-1">
                              <XCircle className="w-3 h-3 text-rose-400" /> FAILED
                            </span>
                          )}
                          {isUnlocked && (
                            <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-mono font-black bg-amber-500/15 text-amber-300 border border-amber-500/30 shadow-sm shadow-amber-500/10 animate-pulse flex items-center gap-1">
                              <Zap className="w-3 h-3 text-amber-400" /> READY
                            </span>
                          )}
                          {isLocked && (
                            <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-mono font-semibold text-slate-500 bg-slate-900/90 border border-slate-800 flex items-center gap-1">
                        ...
                        <div>
                          <h4 className="font-extrabold text-sm text-slate-100">{roundDef.title}</h4>
                          <p className="text-xs text-slate-400 mt-1 leading-relaxed">{roundDef.description}</p>
                        </div>

                        <div className="pt-3 border-t border-slate-800/70 flex items-center justify-between gap-4">
                          <div className="flex items-center gap-1.5 text-[11px] text-slate-400 font-mono font-semibold bg-slate-900/80 px-3 py-1.5 rounded-xl border border-slate-800/80 shrink-0">
                          ...
                          <div className="flex items-center gap-2.5 shrink-0">
                            ...
                            {isUnlocked && (
                              <button
                                ...
                                className="h-9 px-5 rounded-xl font-black text-xs flex items-center gap-2 bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 hover:from-amber-300 hover:to-yellow-300 text-slate-950 shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 active:scale-[0.97] transition-all cursor-pointer whitespace-nowrap shrink-0"
                              >
                            ...
                            {isFailed && (
                              <button
                                onClick={() => onStartRound(roundDef)}
                                className="h-9 px-5 rounded-xl font-black text-xs flex items-center gap-2 bg-gradient-to-r from-rose-600 via-rose-500 to-rose-600 hover:from-rose-500 hover:to-rose-400 text-white shadow-lg shadow-rose-600/30 hover:shadow-rose-600/50 border border-rose-400/40 active:scale-[0.97] transition-all cursor-pointer whitespace-nowrap shrink-0"
                              >
                            ...
                            {isPassed && (
                              <button
                                onClick={() => onStartRound(roundDef)}
                                className="h-9 px-4.5 rounded-xl font-bold text-xs flex items-center gap-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 hover:text-emerald-100 border border-emerald-500/30 hover:border-emerald-500/50 backdrop-blur-md shadow-sm shadow-emerald-500/10 active:scale-[0.97] transition-all cursor-pointer whitespace-nowrap shrink-0"
                              >
                            ...
                            {isLocked && (
                              <div className="h-9 px-4 rounded-xl font-semibold text-xs flex items-center gap-2 bg-slate-900/60 text-slate-500 border border-slate-800/80 cursor-not-allowed opacity-60 select-none whitespace-nowrap shrink-0">
                  ...
                  {progress && progress.rounds.some(r => r.status === 'passed' || r.status === 'failed') && (
                    <motion.div
                      ...
                      className="p-4.5 md:p-5 rounded-3xl border border-amber-500/40 bg-gradient-to-br from-amber-950/30 via-slate-900/90 to-slate-950 backdrop-blur-2xl shadow-2xl space-y-3 mt-1.5"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
