...
export const EliteHRRound: React.FC<EliteHRRoundProps> = ({
  ...
  const startCamera = async () => {
    try {
      ...
      try {
        ...
        console.log('[EliteHRRound] Full round video recording started.');
      } catch (recErr) {
        console.warn('[EliteHRRound] MediaRecorder init note:', recErr);
      }
    } catch (err) {
  ...
  const stopCamera = () => {
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      try {
        ...
        console.warn('[EliteHRRound] Error stopping MediaRecorder:', e);
      }
    }
    if (stream) {
  ...
  // Start directly instead of pre-interview setup for HR round
  useEffect(() => {
    handleConfirmSetupAndStart();
  }, []);
  ...
  const initiateSession = () => {
    ...
    const systemPrompt = `
ROLE: You are an Elite HR Manager and Behavioral Interviewer at ${company.name} conducting the final Round 4 interview.
CATEGORY: ${interviewType.title}
TARGET ROLE: ${role.title}
CURRENT ROUND: ${round.title} (Round 4 of 4)
FOCUS AREAS: ${round.focusAreas.join(', ')}

${rawCleanedContext ? `CANDIDATE CONTEXT / RESUME:\n${rawCleanedContext}` : ''}

=== ROUND 4 — HR & BEHAVIORAL INTERVIEW (EXACT 8-QUESTION ALLOCATION) ===
OBJECTIVE: Evaluate professional behavior, workplace readiness, culture fit, and soft skills.

EXACT 8-QUESTION ALLOCATION (FOLLOW THIS PATTERN STRICTLY):
- Question 1 (Motivation): Warm introduction. Ask why they want this specific internship/role at ${company.name}.
- Question 2 (Motivation): Ask what motivates them professionally or technically in their day-to-day work.
- Question 3 (Teamwork): Ask them to describe a past conflict in a team and how they resolved it.
- Question 4 (Teamwork): Ask how they handle receiving critical or negative feedback from a manager or peer.
- Question 5 (Challenges): Ask them to tell you about a significant technical or professional challenge they faced.
- Question 6 (Challenges): Ask about a specific mistake they made in the past and what they learned from it.
- Question 7 (Learning Mindset): Ask how they approach learning a completely new technology or framework under a tight deadline.
- Question 8 (Availability & Outro): Ask about their availability to join, then deliver a warm closing speech thanking them for completing all 4 rounds!

=== CONVERSATIONAL TONE & NATURAL TRANSITIONS ===
1. WARM CONVERSATIONAL OPENING (QUESTION 1):
   - Open naturally and warmly on your first message (e.g. "Hi! Welcome to your final Round 4 HR interview at ${company.name}. It's great to meet you! To get us started, could you share why you are specifically interested in this ${role.title} role with us?")
   - DO NOT jump straight into cold robotic questions without a warm greeting.

2. NATURAL INTER-QUESTION BRIDGES:
   - Use short, natural 2-3 word conversational bridges before asking follow-up questions based on the candidate's last spoken response (e.g. "Got it!", "Thanks for sharing that!", "That makes sense. Building on that...").
...
4. WRAP-UP & CLOSING SPEECH (ON QUESTION ${totalQuestions}):
   - After candidate answers Question ${totalQuestions}, output a warm, polite closing message thanking them for their time before outputting the final verdict token:
     "Thank you so much for your time today! That completes all ${totalQuestions} questions for Round 4, and concludes your entire interview loop with ${company.name}. We will review all your rounds and get back to you with a final decision soon. Have a wonderful day! [VERDICT: PASSED] [REASON: Candidate successfully completed the HR behavioral questions with professional communication.]"
...
- NEVER ask candidate to write code or type syntax. 
`;
  ...
  const evaluateTranscriptPerformance = (aiVerdict: 'PASSED' | 'FAILED', aiReason?: string): {
    ...
    // 2. Identify HR/Behavioral keywords mentioned in candidate's responses
    const hrKeywords = [
      'team', 'conflict', 'feedback', 'challenge', 'learn', 'goal', 'motivate', 
      'improve', 'mistake', 'resolved', 'communication', 'manager', 'collaboration',
      'deadline', 'adapt', 'culture', 'values', 'passion', 'interested', 'future'
    ];

    const lowerTranscript = fullTranscript.toLowerCase();
    const matchedKeywords = Array.from(new Set(hrKeywords.filter(kw => lowerTranscript.includes(kw))));
    const hrKeywordCount = matchedKeywords.length;

    let commScore = 0;
    let teamworkScore = 0; // Maps to technicalScore
    let confScore = 0;
    let cultureFitScore = 0; // Maps to resumeAuthenticityScore

    if (isAiFail) {
      commScore = Math.min(65, Math.max(35, Math.round(avgWordsPerAnswer) + 20));
      teamworkScore = Math.min(60, Math.max(30, hrKeywordCount * 5 + 30));
      confScore = Math.min(65, Math.max(35, Math.round(wordCount / 4) + 30));
      cultureFitScore = Math.min(60, Math.max(30, hrKeywordCount * 6 + 35));
    } else {
      commScore = Math.min(98, Math.max(75, Math.round(avgWordsPerAnswer * 1.5) + 50));
      teamworkScore = Math.min(96, Math.max(72, hrKeywordCount * 6 + 65));
      confScore = Math.min(98, Math.max(75, Math.round(wordCount / 5) + 60));
      cultureFitScore = Math.min(98, Math.max(78, hrKeywordCount * 5 + 70));
    }

    // Weighted Overall Score (Final HR Score)
    ...
    if (!isAiFail) {
      ...
      if (matchedKeywords.length > 0) {
        strengths.push(`Effective use of behavioral concepts: ${matchedKeywords.slice(0, 4).join(', ')}.`);
      }
      strengths.push("Demonstrated strong cultural fit and teamwork mindset.");
      strengths.push("Articulated clear motivations for joining the company.");

      improvements.push("Ensure examples remain concise and strictly follow the STAR method.");
    } else {
      strengths.push("Showed positive attitude and enthusiasm during screening prompts.");
      strengths.push("Completed the behavioral sequence.");

      improvements.push("Provide specific past examples of teamwork and conflict resolution.");
      improvements.push("Elaborate on 'why' and 'how' when discussing your motivations and challenges.");
    }
    ...
    return {
      ...
      details: {
        ...
        technicalScore: teamworkScore, // Storing teamwork score here
        resumeAuthenticityScore: cultureFitScore, // Storing culture fit score here
        strengths,
        improvements,
        summary: cleanReason
      }
