...
export default function SkillIndiaHub() {
  ...
  const renderIcon = (type: string) => {
    switch (type) {
      ...
      case "settings":
        return <Settings2 className="w-4 h-4 text-slate-300" />;
      case "database":
  ...
  return (
    <div className="min-h-screen w-full bg-[#0b0f17] text-slate-100 selection:bg-emerald-500/20 selection:text-emerald-300 relative overflow-x-hidden pb-16">
      {/* Sleek Header exactly like image, keeping ORIGINAL Voke logo */}
        ...
        <div className="w-full max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-10 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              />
              <span className="font-bold text-xl tracking-tight text-white">Voke</span>
            </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <button
              onClick={() => navigate("/dashboard")}
              className="text-xs font-semibold text-emerald-400 border-b-2 border-emerald-400 pb-0.5 transition-colors"
            >
              Dashboard
            </button>
            ...
            >
              ...
              <span className="text-xs font-medium text-slate-300 group-hover:text-white hidden sm:flex items-center gap-1">
                {userName}
      ...
      <main className="w-full max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-10 py-8 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <span className="px-3 py-0.5 rounded-full text-xs font-medium bg-emerald-950/40 text-emerald-400 border border-emerald-500/40">
                Smart India Hackathon
              </span>
              <span className="text-xs text-slate-400 font-medium">Industry Demand Alignment</span>
            </div>
          ...
          <div className="shrink-0 p-3.5 px-5 rounded-2xl border border-slate-800/80 bg-slate-900/40 flex items-center gap-6 self-start">
            {/* Skill India Brand */}
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-slate-800/80 border border-slate-700/60 flex items-center justify-center p-1">
                <svg viewBox="0 0 48 48" className="w-full h-full" fill="none">
                  {/* Skill India Laptop / Hand Logo Representation */}
                  <rect x="6" y="8" width="36" height="24" rx="3" stroke="#f8fafc" strokeWidth="2.5" />
                  <line x1="2" y1="36" x2="46" y2="36" stroke="#f8fafc" strokeWidth="3" strokeLinecap="round" />
              ...
              <div className="text-left">
                <div className="text-xs font-bold text-white tracking-tight leading-none">Skill India</div>
                <div className="text-[9px] text-slate-400 font-medium mt-0.5">कौशल भारत - कुशल भारत</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-slate-800/80 border border-slate-700/60 flex items-center justify-center p-1">
                <svg viewBox="0 0 40 40" className="w-full h-full" fill="none">
                  ...
                  <path d="M20 4C17 4 15 7 15 10C15 12 16 14 18 15C16 16 14 18 14 21C14 23 15 25 17 26V30H23V26C25 25 26 23 26 21C26 18 24 16 22 15C24 14 25 12 25 10C25 7 23 4 20 4Z" fill="#e2e8f0" />
                  <circle cx="20" cy="33" r="3.5" stroke="#10b981" strokeWidth="1.5" />
                  <rect x="13" y="37" width="14" height="2" rx="1" fill="#e2e8f0" />
                </svg>
              </div>
              <div className="text-left">
                <div className="text-xs font-bold text-white tracking-tight leading-none">iGOT Karmayogi</div>
                <div className="text-[9px] text-slate-400 font-medium mt-0.5">Government of India</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 p-1 bg-slate-900/60 rounded-xl border border-slate-800/80 w-full">
          {ROLES_DATA.map((r) => (
            <button
              key={r.id}
              onClick={() => {
              ...
              className={`py-2 px-3 rounded-lg text-xs sm:text-sm font-medium transition-all text-center ${
                activeRoleId === r.id
                  ? "bg-emerald-950/40 text-white border border-emerald-500/40 font-semibold"
        ...
        {/* Hero Role Card */}
        <div className="p-6 rounded-2xl border border-slate-800/80 bg-slate-900/40 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 w-full">
          {/* Left: Role Info with </> Green Icon Box */}
          <div className="flex items-center gap-4 min-w-[320px]">
            <div className="w-13 h-13 rounded-xl bg-emerald-950/60 border border-emerald-500/30 flex items-center justify-center text-emerald-400 font-mono text-xl font-bold shrink-0 p-3">
              {role.heroIconText}
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm text-slate-400 flex items-center gap-2.5">
                <span>Target CTC: <strong className="text-slate-200 font-semibold">{role.targetCtc}</strong></span>
                <span>•</span>
          ...
          <div className="flex-1 max-w-xl space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-400 font-medium">Industry Hiring Bar Readiness</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2.5">
              {role.criticalGaps.map((skill, idx) => (
                <div
                  key={idx}
                  className="p-3.5 rounded-xl border border-slate-800/80 bg-slate-900/40 flex items-center justify-between gap-4 text-xs sm:text-sm hover:border-slate-700 transition-colors group cursor-default"
                >
                  ...
                  <div className="flex items-center gap-2.5 shrink-0">
                    <span className="px-2.5 py-0.5 rounded text-[11px] font-mono bg-rose-950/40 text-rose-400 border border-rose-800/60 font-medium">
            ...
            <div className="pt-2">
              ...
              {showAlignedSkills && (
                <div className="mt-2.5 space-y-2 pl-3 border-l-2 border-slate-800">
                  {role.alignedSkills.map((s, idx) => (
                    <div key={idx} className="flex items-center gap-2.5 text-xs text-slate-400 py-1">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2.5">
              ...
              <div className="p-4 rounded-xl border border-slate-800/80 bg-slate-900/40 space-y-2.5 hover:border-slate-700 transition-colors">
              ...
              <div className="p-4 rounded-xl border border-slate-800/80 bg-slate-900/40 space-y-2.5 hover:border-slate-700 transition-colors">
                ...
                <div className="pt-1 flex justify-end">
                  <Button
                    ...
                    className="text-xs h-8 bg-slate-900 border-slate-700/80 hover:bg-slate-800 text-slate-200 gap-1.5 rounded-lg font-medium"
                  >
      ...
      <div className="fixed bottom-0 left-0 pointer-events-none z-20">
        ...
        <div className="absolute bottom-4 left-6 pointer-events-auto">
          <div className="p-2.5 px-3.5 rounded-xl border border-slate-800/90 bg-[#090d16]/90 backdrop-blur-md shadow-2xl flex items-center gap-2.5">
      ...
      <Dialog open={diagnosticModalOpen} onOpenChange={setDiagnosticModalOpen}>
        <DialogContent className="max-w-sm bg-[#0d131f] border-slate-800 text-slate-100">
          <DialogHeader>
          ...
          <div className="space-y-2 py-2 text-xs">
            <div className="p-3 rounded-lg bg-slate-900/60 border border-slate-800 space-y-1.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-emerald-400">{role.readinessScore}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Badge:</span>
                <span className="font-medium text-slate-300">Verifiable Digilocker ID</span>
              </div>
            </div>
          </div>
