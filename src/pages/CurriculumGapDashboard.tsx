...
const PRESET_CURRICULA: CurriculumPreset[] = [
  {
    id: "aicte-2024",
    name: "AICTE Model Curriculum 2024",
    university: "National Technical Framework",
    description: "Standard national curriculum used by affiliating colleges and autonomous institutes.",
    alignment: 58
  },
  {
    id: "anna-univ-2021",
    name: "Anna University (2021 Reg)",
    university: "State Affiliated Engineering",
    description: "Traditional core engineering syllabus with emphasis on theoretical foundations.",
    alignment: 52
  },
  {
    id: "aktu-2023",
    name: "AKTU / State Tech Scheme",
    university: "State Technical Board",
    description: "Standard B.Tech CSE framework with semester laboratory exercises.",
    alignment: 55
  },
  {
    id: "tier2-autonomous",
    name: "Autonomous Engineering College",
    university: "Autonomous Framework",
    description: "Custom college syllabus with early elective offerings.",
    alignment: 64
  }
];

// Concise, tag-driven data structure
interface GapItem {
  id: string;
  subject: string;
...
export default function CurriculumGapDashboard() {
  ...
  const [inputMode, setInputMode] = useState<"preset" | "upload" | "paste">("preset");
  const [selectedPresetId, setSelectedPresetId] = useState<string>("aicte-2024");
  const [activeCurriculumTitle, setActiveCurriculumTitle] = useState("AICTE Model Curriculum 2024");
  ...
  const handleStartAnalysis = (presetTitleOverride?: string) => {
    setIsAnalyzing(true);

    let titleToUse = presetTitleOverride;
    if (!titleToUse) {
      if (inputMode === "preset") {
        const p = PRESET_CURRICULA.find(c => c.id === selectedPresetId);
        titleToUse = p ? p.name : "Selected University Curriculum";
      } else if (inputMode === "upload") {
        titleToUse = uploadedFile ? uploadedFile.name.replace(/\.[^/.]+$/, "") : "Uploaded Syllabus Document";
      } else {
        titleToUse = "Custom Department Curriculum";
      }
    }

    setTimeout(() => {
      setActiveCurriculumTitle(titleToUse || "Academic Curriculum");
      setIsAnalyzing(false);
      setHasAnalyzed(true);
      toast.success("Curriculum audit complete! 9 subjects benchmarked.");
    }, 1100);
  };
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#090d16] text-slate-900 dark:text-slate-100 transition-colors pb-16">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        {/* STEP 1: INITIAL CURRICULUM INPUT SCREEN (When not yet analyzed) */}
        {!hasAnalyzed ? (
          <div className="max-w-3xl mx-auto pt-6 space-y-6">
            <div className="text-center space-y-2">
              ...
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
                Audit Your Institution's Curriculum
              </h1>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
                Add or select your college syllabus to benchmark course content against real-world 2025/2026 tech company hiring criteria.
              </p>
            </div>

            <Card className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 shadow-sm overflow-hidden">
              <div className="p-1.5 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-center gap-2">
                <Button
                  variant={inputMode === "preset" ? "secondary" : "ghost"}
                  size="sm"
              ...
              <CardContent className="p-5 sm:p-6 space-y-5">
                ...
                {inputMode === "preset" && (
                  <div className="space-y-3">
                    <span className="block text-xs font-medium text-slate-700 dark:text-slate-300">
                      Select University / Autonomous Framework:
                    </span>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {PRESET_CURRICULA.map((preset) => (
                        <div
                          ...
                          className={`p-3.5 rounded-lg border transition-all cursor-pointer text-xs space-y-1 ${
                            selectedPresetId === preset.id
                              ? "border-emerald-500 bg-emerald-50/40 dark:bg-emerald-950/20 shadow-xs"
                              : "border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 hover:border-slate-300"
                          }`}
                        >
                ...
                {inputMode === "upload" && (
                  <div className="space-y-3">
                    ...
                    <label className="border-2 border-dashed border-slate-200 dark:border-slate-800 hover:border-emerald-500/60 rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-colors bg-slate-50/50 dark:bg-slate-900/30">
                      <UploadCloud className="w-8 h-8 text-slate-400 mb-2" />
                      <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                        {uploadedFile ? uploadedFile.name : "Click to select syllabus PDF or Drag & Drop"}
                      </span>
                      <span className="text-[11px] text-slate-400 mt-1">
                        Supports departmental syllabus PDFs up to 25MB
                      </span>
                      <input
                        type="file"
                        accept=".pdf,.txt,.docx"
                ...
                {inputMode === "paste" && (
                  <div className="space-y-2">
                    <span className="block text-xs font-medium text-slate-700 dark:text-slate-300">
                      Paste Syllabus Course Content / Units:
                    </span>
                    <Textarea
                      rows={5}
                      value={pastedText}
                      onChange={(e) => setPastedText(e.target.value)}
                      placeholder="e.g., Unit 1: Database Management Systems, Normalization, SQL CRUD... Unit 2: Computer Networks OSI Model..."
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <span className="block text-[11px] text-slate-400 mb-1">Target Department</span>
                  ...
                  <div>
                    <span className="block text-[11px] text-slate-400 mb-1">Target Hiring Benchmark</span>
                  ...
                  <div>
                    <span className="block text-[11px] text-slate-400 mb-1">Target Batch / Semester</span>
                ...
                <div className="pt-2">
                  <Button
                    ...
                    className="w-full h-10 bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs sm:text-sm gap-2"
                  >
                    {isAnalyzing ? (
                      <>
                        <RefreshCw className="w-4 h-4 animate-spin" />
                        <span>Benchmarking Against 2026 Hiring Bar...</span>
                      </>
                    ) : (
                      <>
          ...
          /* STEP 2: DASHBOARD COMPARISON VIEW (Rendered after curriculum is submitted) */
          <>
            {/* Active Curriculum Banner */}
            <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
                <div className="text-xs">
                  <span className="text-slate-400">Audited Curriculum: </span>
                  <span className="font-semibold text-slate-800 dark:text-slate-100">
                    {activeCurriculumTitle}
                  </span>
                  <span className="text-slate-400 ml-2">
                    ({department.toUpperCase()} • {semester.toUpperCase()} • {targetBenchmark === "product" ? "Tier-1 Product" : targetBenchmark})
                  </span>
                </div>
              </div>
              ...
              <Button
                ...
                className="h-7 text-xs border-slate-200 dark:border-slate-800 self-start sm:self-auto"
              >
                <RefreshCw className="w-3 h-3 mr-1" />
                <span>Change / Re-upload Curriculum</span>
              </Button>
            </div>
          ...
          <div className="space-y-1.5 py-2 text-xs">
            <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{activeCurriculumTitle}</span>
              </div>
              <div className="flex justify-between">
