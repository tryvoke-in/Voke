...
import { 
  ArrowLeft, Copy, Play, BookOpen, Link, Check, X, FileText, ArrowUpRight, ChevronDown, ChevronUp
} from "lucide-react";
...
const CollegeDriveDetails: React.FC = () => {
  ...
  const interviewLink = drive.interviewUrl || `${window.location.origin}/adaptive-interview?role=${encodeURIComponent(drive.targetRole)}&driveId=${drive.id}`;

  const customQuestions = drive.customQuestions && drive.customQuestions.length > 0
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mb-2">
              <Badge className={`text-[10px] px-2 py-0.5 uppercase tracking-wider ${
                drive.status === "active" ? "bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border-emerald-500/30" :
                drive.status === "scheduled" ? "bg-blue-500/20 text-blue-700 dark:text-blue-300 border-blue-500/30" :
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Target Role: <strong className="text-foreground">{drive.targetRole}</strong> • Passing Benchmark: <strong className="text-emerald-600 dark:text-emerald-400">{drive.passingScore}%</strong> • Duration: {drive.durationMinutes} mins
            </p>
          </div>
        </div>
          ...
          <div className="p-4 rounded-xl bg-blue-50/50 dark:bg-blue-950/25 border border-blue-200 dark:border-blue-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1 min-w-0 flex-1">
              <div className="text-sm font-semibold text-blue-700 dark:text-blue-300 flex items-center gap-1.5">
                <Link className="w-4 h-4 text-blue-600 dark:text-blue-400" /> Direct Candidate Assessment Link
              </div>
            ...
            {isQuestionsExpanded && (
              <div className="space-y-3 animate-in slide-in-from-top-2 fade-in duration-200">
                {customQuestions.map((q, idx) => (
                  <div key={idx} className="p-3.5 rounded-lg bg-muted/40 dark:bg-black/40 border border-border/50 text-sm space-y-1.5">
                    ...
                    {q.expectedAnswerOrKeyPoints && (
                      <p className="text-xs text-muted-foreground pl-0 md:pl-20 mt-1">
                        <strong className="text-blue-700 dark:text-blue-300">Expected:</strong> {q.expectedAnswerOrKeyPoints}
                      </p>
                    )}
                  </div>
                ))}
          ...
          <div className="p-5 rounded-xl bg-card border border-border shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <div className="text-sm text-muted-foreground bg-muted/50 px-3 py-1 rounded-md border border-border/50">
                Threshold: <span className="text-emerald-600 dark:text-emerald-400 font-bold">{drive.passingScore || 75}%</span>
              </div>
            </div>
              ...
              <Table>
                <TableHeader className="bg-muted/40 dark:bg-muted/30">
                  <TableRow className="border-border">
                    <TableHead className="text-xs font-semibold text-foreground/80">Candidate Email</TableHead>
                  ...
                  {drive.candidates && drive.candidates.length > 0 ? (
                    drive.candidates.map(candidate => {
                      ...
                      const isCandidatePassed = hasAttempted && (
                        ...
                        (candidate.score !== undefined && candidate.score >= (drive.passingScore || 75))
                      );

                      return (
                        <TableRow key={candidate.studentEmail} className="border-border/50 transition-colors hover:bg-muted/30">
                          <TableCell className="font-mono text-foreground">
                            <div className="font-semibold text-sm text-foreground">{candidate.studentName || candidate.studentEmail.split('@')[0]}</div>
                            <div className="text-xs text-muted-foreground mt-0.5">{candidate.studentEmail}</div>
                          </TableCell>
                          <TableCell>
                            <Badge className={`text-[10px] uppercase tracking-wider ${candidate.status === "Completed"
                          ...
                          <TableCell className="text-right">
                            {hasAttempted ? (
                              ...
                              >
                                <FileText className="w-3 h-3 mr-1.5" /> AI Report
                              </Button>
                            ) : (
            ...
            <div className="py-4 space-y-5">
              ...
              <div className="p-4 rounded-xl bg-muted/30 dark:bg-muted/20 border border-border flex items-center justify-between gap-4">
                ...
                <div className="text-right p-3 bg-background rounded-lg border border-border shadow-sm min-w-[120px]">
                  <div className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider mb-1">Overall AI Score</div>
                  <div className={`text-3xl font-extrabold font-mono ${(selectedCandidateForReport.candidate.score || 0) >= (selectedCandidateForReport.drive.passingScore || 75)
              ...
              <div className="p-4 rounded-xl bg-blue-50/50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/50 text-sm space-y-2">
                <span className="font-bold text-blue-800 dark:text-blue-300 flex items-center gap-1.5">
                  <ArrowUpRight className="w-4 h-4" /> Placement Cell Assessment Summary:
                </span>
              ...
              {selectedCandidateForReport.candidate.answers && selectedCandidateForReport.candidate.answers.length > 0 && (
                <div className="space-y-4 pt-2">
                  ...
                  {selectedCandidateForReport.candidate.answers.map((ans, idx) => (
                    <div key={idx} className="p-4 rounded-xl bg-card border border-border shadow-sm space-y-3 text-sm transition-all hover:border-blue-500/30">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                        ...
                        <Badge className={`shrink-0 text-[11px] font-mono font-bold px-2.5 py-1 ${ans.score >= (selectedCandidateForReport.drive.passingScore || 75)
                      ...
                      <div className="bg-muted/30 dark:bg-muted/20 p-3 rounded-lg border border-border/50 text-foreground/90 ml-8">
                        <strong className="text-muted-foreground block text-[10px] uppercase tracking-wider mb-1">Submitted Answer:</strong>
                        <p className="italic text-foreground/80 text-xs md:text-sm">{ans.studentAnswer}</p>
                      </div>
                      ...
                      <div className="p-3 rounded-lg bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 text-blue-800 dark:text-blue-200 ml-8">
                        <strong className="text-blue-600 dark:text-blue-400 block text-[10px] uppercase tracking-wider mb-1">AI Feedback:</strong>
                        <p className="text-xs md:text-sm">{ans.aiFeedback}</p>
                      </div>
                    </div>
                  ))}
                </div>
